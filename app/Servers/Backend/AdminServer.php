...
class AdminServer extends CommonServer
{
    ...
    public function store($input)
    {
        ...
        $list = $this->admin->getList($search_where);
        if (!empty($list)) {
            $code = $list->username == $username ? 'x00001' : 'x00002';
            return ['code' => [$code, 'system']];
        }

        $result['list'] = $this->admin->store($username, $email, $password, $permission_id, $status);
    ...
    public function update($id, $input)
    {
        ...
        $result = $this->admin->update($id, $username, $email, $password, $permission_id, $status);
